...
from schemas.book import BookCreate, BookResponse, BookStatus, BookUpdate, PaginatedBooks
...
@router.put(
    ...
    response_model=BookResponse,
    status_code=status.HTTP_200_OK,
    summary="Оновити книгу (повна заміна полів)",
)
async def update_book(
    ...
    try:
        doc = await service.update_book(book_id, book_data)
    except InvalidBookIdError as exc:
        # Некоректний формат id → 400 (так само, як у GET/{id} і DELETE).
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))

    if doc is None:
        # Книги з таким id не існує — оновлювати нічого.
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Книгу з id={book_id} не знайдено",
        )
