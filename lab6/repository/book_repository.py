...
class BookRepository:
    ...
    async def list_books(
        ...
        """
        Повертає (список_книг, next_cursor).

        Cursor-based пагінація:
          1) Сортуємо за парою (sort_field, id) — стабільно і детерміновано
          2) Якщо отриманий cursor — фільтруємо WHERE (sort_field, id) > (last_value, last_id)
          3) Беремо limit+1 — якщо прийшло більше, ніж limit, значить є наступна сторінка,
             і ми формуємо next_cursor з останнього "корисного" елемента
          4) Коли наступної сторінки нема — next_cursor = None
        """
        if sort_by == "title":
