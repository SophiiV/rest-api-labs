...
@pytest.mark.asyncio
async def test_get_books_without_token_returns_401(client: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_book_by_id_without_token_returns_401(client: AsyncClient):
    r = await client.get(f"{BASE_URL}/00000000-0000-0000-0000-000000000000")
    assert r.status_code == 401


@pytest.mark.asyncio
async def test_books_with_invalid_token_returns_401(client: AsyncClient):
    r = await client.get(BASE_URL, headers={"Authorization": "Bearer garbage"})
    assert r.status_code == 401
...
# ----------------------------------------------------------------------
# Фільтрація і сортування (із токеном)
# ----------------------------------------------------------------------
